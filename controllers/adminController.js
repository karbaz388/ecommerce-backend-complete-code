...
export const deleteUser = catchAsyncError(async (req, res, next) => {
  ...
  const deleteRequestedUser = await database.query(`DELETE FROM users WHERE id = $1 RETURNING *`, [
    id,
  ]);

  if (deleteRequestedUser.rows.length === 0) {
    return next(new ErrorHandler('User not found.', 404));
  }

  const avatar = deleteRequestedUser.rows[0].avatar;
  ...
  res.status(200).json({
    ...
    user: deleteRequestedUser.rows[0],
  });
});
